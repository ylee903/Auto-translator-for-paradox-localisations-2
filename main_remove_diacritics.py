...
This version:
- Reads .yml/.yaml files from an input folder (default: ./diacritics_to_be_removed)
- Writes transformed files to an output folder (default: ./diacritics_removed)
- For each line like:

      c_guangdong: "gwóngdūngyàhn"

  it extracts the value ("gwóngdūngyàhn"), calls remove_diacritics()
  to strip diacritics, and replaces the value, e.g.:

      c_guangdong: "gwongdungyahn"

- Uses:
    - remove_diacritics.py in the same directory
...
DEFAULT_INPUT_DIR = Path("diacritics_to_be_removed")
DEFAULT_OUTPUT_DIR = Path("diacritics_removed")
...
def make_replacer():
    ...
    that calls remove_diacritics() on the original value.
    """
    ...
    def replacewithrealname1(original_text: str) -> str:
        ...
        try:
            stripped = remove_diacritics(original_text)
            # If nothing changed, it's still fine – we just return it
            return stripped
        except Exception as e:
            print(
                f"[ERROR] remove_diacritics() failed for value {original_text!r}: {e}\n"
...
def process_line(line: str, replacer) -> str:
    ...
    Example:
        input:  c_guangdong: "gwóngdūngyàhn"
    ...
    # Build the new value using the replacer (diacritic removal)
...
def main(argv=None):
    """
    Usage:
        python main_remove_diacritics.py [input_folder] [output_folder]

    Defaults:
        input_folder  = ./diacritics_to_be_removed
        output_folder = ./diacritics_removed
    """
    if argv is None:
        argv = sys.argv[1:]
